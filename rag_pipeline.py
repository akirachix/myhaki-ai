...
# --- Embedding Model Configuration (Lazy Loading) ---
MODEL_DIR = os.getenv("TRANSFORMERS_CACHE", "/app/model_cache")
os.makedirs(MODEL_DIR, exist_ok=True)
embedding_model_name = "nlpaueb/legal-bert-base-uncased"
VECTOR_SIZE = int(os.getenv("VECTOR_SIZE", 768))

# Use functools.lru_cache to ensure the model is loaded only once
@functools.lru_cache(maxsize=1)
def get_embedding_model():
    """Lazily loads the embedding model and tokenizer and caches it."""
    print(f"Loading model '{embedding_model_name}' from cache or downloading...")
